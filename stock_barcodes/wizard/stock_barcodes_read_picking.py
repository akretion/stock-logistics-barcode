...
class WizStockBarcodesReadPicking(models.TransientModel):
    ...
    def _process_stock_move_line(self):  # noqa: C901
        ...
        if (
            ...
            return False
        if self.option_group_id.barcode_guided_mode == "flexible":
            if not lines:
                self._set_messagge_info(
                    "not_found", _("Scanned lot is not the expected lot")
                )
                return False
            if not lines.filtered(lambda x: not x.picked):
                self._set_messagge_info(
                    "more_match",
                    _("Quantities scanned are higher than necessary for scanned lot."),
                )
                return False
        move_lines_dic = {}
